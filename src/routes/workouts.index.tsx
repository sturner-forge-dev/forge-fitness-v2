		...
		<main className='page-wrap px-4 pb-12 pt-14'>
			<div className='rise-in mb-6 flex items-end justify-between'>
				<div>
					...
					<h1 className='display-title text-4xl font-bold tracking-tight text-(--sea-ink)'>Workouts</h1>
				</div>
				<Button asChild>
			...
			{loading ? (
				...
				<div className='island-shell rise-in rounded-2xl px-6 py-16 text-center'>
					<p className='text-sm font-medium text-(--sea-ink)'>No workouts logged yet</p>
					<p className='mt-1 text-xs text-(--sea-ink-soft)'>Your sessions will appear here after you finish one.</p>
				...
				<div className='rise-in space-y-3'>
					{sessions.map((session) => {
						const totalSets = session.exercises.reduce((acc, ex) => acc + ex._count.sets, 0);
						...
						return (
							<article key={session.id} className='island-shell feature-card rounded-2xl p-5'>
								<div className='flex items-start justify-between gap-4'>
									<div>
										<p className='font-semibold text-(--sea-ink)'>{session.name ?? 'Workout'}</p>
									...
									<div className='flex gap-4 text-right'>
										<div>
											<p className='text-sm font-bold text-(--sea-ink)'>{session._count.exercises}</p>
